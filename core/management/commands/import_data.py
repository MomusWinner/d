...
from core.models import Order, OrderProduct, PickupPoint, Product, Role, Supplier, User
...
class Command(BaseCommand):
    def handle(self, *args: Any, **options: Any) -> str | None:
        ...
        # pickup points
        with open(os.path.join(base_path, "pp.csv"), encoding="utf-8-sig") as f:
        ...
        with open(os.path.join(base_path, "orders.csv"), encoding="utf-8-sig") as f:
            ...
            for row in reader:
                ...
                order, created = Order.objects.get_or_create(
                    id=row["id"],
                    defaults={
                        "order_date": process_date(row["order_date"]),
                        "delivery_date": process_date(row["delivery_date"]),
...
def process_date(src_date):
    try:
        # Преобразуем в правильный формат
        date_obj = datetime.strptime(src_date, "%m/%d/%Y")
        validated_date = date_obj.strftime("%Y-%m-%d")
        # Используйте validated_date для сохранения`
    except ValueError:
        # Обработка ошибки
